...
// TODO kk:20200106 - Must be here as a source code due to version conflict. Remove once the developers resolve it.
// kk:20200106 - Note that without "--warnaserror+:25" (which I consider a must for F# projects) and with #nowarn "25" the code below compiles...
...
module IntMap =
    ...
    let rec find k m =
        let notFound() = failwith <| $"IntMap.find: key %d{k} is not an element of the map"
        match m with
